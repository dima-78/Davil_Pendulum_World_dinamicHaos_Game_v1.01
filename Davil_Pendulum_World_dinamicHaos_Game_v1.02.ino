...
// маятник B
float angle1B = PI / 2, angle2B = PI / 4; // углы маятника B
float angularVel1B = 0, angularVel2B = 0; // угловые скорости
float angularAcc1B = 0, angularAcc2B = 0; // угловые ускорения
const float length1B = 25, length2B = 25; // длины
float mass1B = 1.0, mass2B = 1.0;         // массы
...
// анимация хаотичных маятников
void drawChaoticPendulums() {
    // вычисление позиций маятника A
    int x1A = arduboy.width() / 2 + length1 * sin(angle1);
    int y1A = length1 * cos(angle1);
    int x2A = x1A + length2 * sin(angle2);
    int y2A = y1A + length2 * cos(angle2);

    // вычисление позиций маятника B
    int x1B = arduboy.width() / 2 + length1B * sin(angle1B);
    int y1B = arduboy.height() - length1B * cos(angle1B);
    int x2B = x1B + length2B * sin(angle2B);
    int y2B = y1B - length2B * cos(angle2B);

    // проверка столкновения концов маятников
    if (abs(x2A - x2B) < 4 && abs(y2A - y2B) < 4) {
        float tempVelA = angularVel2;
        float tempVelB = angularVel2B;

        angularVel2 = tempVelB * 0.8; // эффект отскока
        angularVel2B = tempVelA * 0.8;
    }

    // отображение маятников
    arduboy.drawLine(arduboy.width() / 2, 0, x1A, y1A, WHITE);
    arduboy.drawLine(x1A, y1A, x2A, y2A, WHITE);
    arduboy.fillCircle(x2A, y2A, 3, WHITE);

    arduboy.drawLine(arduboy.width() / 2, arduboy.height(), x1B, y1B, WHITE);
    arduboy.drawLine(x1B, y1B, x2B, y2B, WHITE);
    arduboy.fillCircle(x2B, y2B, 3, WHITE);
}


// логика обновления маятника B
void updatePendulumB() {
    float num1 = -g * (2 * mass1B + mass2B) * sin(angle1B);
    float num2 = -mass2B * g * sin(angle1B - 2 * angle2B);
    float num3 = -2 * sin(angle1B - angle2B) * mass2B;
    float num4 = angularVel2B * angularVel2B * length2B + angularVel1B * angularVel1B * length1B * cos(angle1B - angle2B);
    float den = length1B * (2 * mass1B + mass2B - mass2B * cos(2 * angle1B - 2 * angle2B));
    float angularAcc1B = (num1 + num2 + num3 * num4) / den;

    num1 = 2 * sin(angle1B - angle2B);
    num2 = angularVel1B * angularVel1B * length1B * (mass1B + mass2B);
    num3 = g * (mass1B + mass2B) * cos(angle1B);
    num4 = angularVel2B * angularVel2B * length2B * mass2B * cos(angle1B - angle2B);
    den = length2B * (2 * mass1B + mass2B - mass2B * cos(2 * angle1B - 2 * angle2B));
    float angularAcc2B = (num1 * (num2 + num3 + num4)) / den;

    angularVel1B += angularAcc1B * dt;
    angularVel2B += angularAcc2B * dt;
    angle1B += angularVel1B * dt;
    angle2B += angularVel2B * dt;
}
...
void drawGameOverScreen() {
    ...
    // Проверяем, завершена ли игра победой
    if (level > 3) {
        // показываем сообщение о победе в течение 2 секунд
        if (showFinishMessage) {
            arduboy.setCursor(26, 24);
            arduboy.print(F("YOU WIN HAOS!(;"));

            // Проверяем, прошло ли 2 секунды
            if (millis() - levelStartTime > 3000) {
                showFinishMessage = false; // завершаем показ сообщения
            }
        } else if (level > 3 && !showFinishMessage) {
            fightPendulums(); // анимация маятников
        }
    } else {
...
void drawMenu() {
  ...
  arduboy.print(menuOption == 1 ? "> SettingsHaos" : "  SettingsHaos");

  arduboy.setCursor(20, 40);
  arduboy.print(menuOption == 2 ? "> ScoresHaos" : "  ScoresHaos");
}
